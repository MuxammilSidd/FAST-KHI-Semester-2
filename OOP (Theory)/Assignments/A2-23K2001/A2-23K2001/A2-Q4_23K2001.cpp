...
virtual void addComment(post *p) { p->addComment(); }
...
void displayAcc() override { /*overriding original from the base user class*/
    cout<<"\n(Regular user)";
    user::displayAcc();
}
};
...
void addComment(post *p) override { /*overriding original from the base user class*/
    cout<<"(sponsored comment)"<<endl;
    p->addComment(); 
}
};
